...
describe Vines::Stream::Server::Outbound::Authoritative do
  ...
  def test_valid_verification
    node = node('<db:verify from="remote.host" to="local.host" id="1234" type="valid"/>')
    @stream.expect(:router, @router)
    # NOTE this tests the 'inbound' stream var
    @stream.expect(:write, nil, ["<db:result from='#{node[:to]}' to='#{node[:from]}' type='#{node[:type]}'/>"])
  ...
  def test_invalid_verification
    node = node('<db:verify from="remote.host" to="local.host" id="1234" type="invalid"/>')
    @stream.expect(:router, @router)
    # NOTE this tests the 'inbound' stream var
    @stream.expect(:write, nil, ["<db:result from='#{node[:to]}' to='#{node[:from]}' type='error'><error type='cancel'><item-not-found xmlns='urn:ietf:params:xml:ns:xmpp-stanzas'/></error></db:result>"])
